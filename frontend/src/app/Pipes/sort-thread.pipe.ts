...
export class SortThreadPipe implements PipeTransform {
	...
	transform(value: thread[], ...args: unknown[]): thread[] {
		let temp = [...value]
		return temp.sort((a, b) => this.score(a.replycount, a.created_at) - this.score(b.replycount, b.created_at))
	...
	score(r: number, t: string): number {
		let time = Math.trunc((Date.now() - new Date(t).getTime()) / (1000*60*60))
		let constant = 1
		return (r+50)*(0.9**time)
	}
